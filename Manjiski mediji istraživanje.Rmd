...
```{r}
 ui<- mainPanel( p("Objavljivanje ovog istraživanja pomogla je Misija OEBS-a u Srbiji. Stavovi izraženi u ovom istraživanju pripadaju isključivo autoru i ne predstavljaju zvaničan stav Misije OEBS-a u Srbiji."),
                 p(" Na ovoj stranici možete da skinete prikupljene podatke u .csv i .xlxs formatu klikom na dugme Preuzmi. Takođe, ukoliko mislite da podaci treba da se ažuriraju slobodno kontaktirajte autora na sledeću", a("email adresu.",href="mailto:nedim.sejdinovic@gmail.com"),align="justify"),
                 
   downloadButton (outputId = "downloaddatabase", label = "Preuzmi.CSV", class="butt",
                                   tags$head(tags$style(" .butt{background-color:#0e7ceb;} .butt{color: black;} "))),
                        
  downloadButton ("downloaddatabase1", "Preuzmi.XLSX",class = "butt",
                                   tags$head(tags$style(" .butt{background-color:#0e7ceb;} .butt{color: black;} "))))
                        
server <- function (input, output) {output$downloaddatabase <- downloadHandler (
    filename = function(){paste("ManjinskimedijiNSNM-", Sys.Date(), ".csv",sep = "")},
    
    content = function(file) {
      write.csv(minoritymedia, file)}
    )
 output$downloaddatabase1 <- downloadHandler(
    filename = function(){paste("ManjinskimedijiNSNM-", Sys.Date(), ".xlsx", sep = "")},
...
![](/Users/jojo/Desktop/R trainings various/Minority media/OSCE.png) 
